...
class TestNeuralNetwork(unittest.TestCase):
    ...
    def test_evaluate(self):
        # Test if the evaluate function works correctly
        nx = 3
        nodes = 2
        nn = NN(nx, nodes)
        ...
        # Perform forward propagation and evaluate
        nn.forward_prop(X)
        predictions, cost = nn.evaluate(X, Y)

        # Check if predictions are either 0 or 1
    ...
    def test_gradient_descent(self):
        # Test if gradient descent works correctly
        nx = 3
        nodes = 2
        nn = NN(nx, nodes)
        ...
        # Store the initial weights before gradient descent
        W1_before = nn.W1.copy()
        W2_before = nn.W2.copy()

        # Perform one step of gradient descent
        nn.gradient_descent(X, Y, A1, A2, alpha=0.05)

        # Check if weights have been updated
        self.assertFalse(np.all(W1_before == nn.W1))
        self.assertFalse(np.all(W2_before == nn.W2))
